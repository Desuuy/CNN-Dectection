...
# Định nghĩa lại lớp ResNet18TwoBranch giống như trong mã huấn luyện
...
model = torch.load("E:/Nhom10_PhanMemNhanDang5VatDungTrongNha/6.ChuongTrinh/Weights/model.pth", map_location=torch.device('cpu'), weights_only=False)

model.eval()  # Chuyển mô hình sang chế độ đánh giá (evaluation mode)

# Ánh xạ class_id sang tên lớp
class_names = {0: "Background", 1: "Bowl", 2: "Chopsticks", 3: "Glass", 4: "Knife", 5: "Spoon"}
...
if uploaded_file is not None:
    ...
    if score > 0.5:  # Chỉ hiển thị các dự đoán có độ tin cậy cao
        ...
        try:
            font = ImageFont.truetype("arial.ttf", 50)  # Font Arial, kích thước 30
        except:
        ...
        # Thêm padding cho nền của nhãn (để đẹp hơn)
        padding = 5
